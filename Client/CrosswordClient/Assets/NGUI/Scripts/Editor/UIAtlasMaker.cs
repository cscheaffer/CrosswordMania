...
public class UIAtlasMaker : EditorWindow
{
	...
	void OnGUI ()
	{
        ...
        // jwu NGUI - prevents ReferenceAtlas from being able to accept changes to real Atlases
        if (NGUISettings.atlas != null &&
            (NGUISettings.atlas.replacement != null || NGUISettings.atlas.name.EndsWith("RefAtlas")))
        {
            Debug.LogError(NGUISettings.atlas.name + " is a Reference Atlas. Add assets to normal atlases only.");
