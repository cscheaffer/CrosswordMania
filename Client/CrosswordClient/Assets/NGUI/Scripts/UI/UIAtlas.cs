...
public class UIAtlas : MonoBehaviour
{
    ...
    public void SpecialReplacement(UIAtlas replacementAtlas)
    {
        mReplacement = replacementAtlas;
        ...
        for (int i = 0, imax = list.Length; i < imax; ++i)
        {
            ...
            if (sp.gameObject.name == "CardBackground")
            {
                Debug.LogError("SpecialReplacement() Breakpoint");
            }

            if (sp != null && sp.atlas != null && sp.atlas == this)
            {
                UIAtlas atl = sp.atlas;
                sp.atlas = null;
                sp.atlas = atl;
        ...
        for (int i = 0, imax = fonts.Length; i < imax; ++i)
        {
            ...
            if (font != null && font.atlas != null && font.atlas == this)
            {
                UIAtlas atl = font.atlas;
                font.atlas = null;
                font.atlas = atl;
        ...
        for (int i = 0, imax = labels.Length; i < imax; ++i)
        {
            ...
            if (lbl.bitmapFont != null && lbl.bitmapFont.atlas != null && lbl.bitmapFont.atlas == this)
            {
                UIFont font = lbl.bitmapFont;
                lbl.bitmapFont = null;
                lbl.bitmapFont = font;
    ...
    //JWU - Added code here for RefAtlas
    public void FixUpAtlas()
    {
        if (mReplacement == null)
        {
            if (name != null)
            {
                try
                {
                    var atlasId = name.Replace("(Clone)", "").Replace("RefAtlas", "Atlas");
                    //mReplacement = AtlasManager.Instance.GetActiveUiAtlas(atlasId);
                }
                catch (Exception)
                {
                    //NotLogging this because we don't actually care
                    //Debug.LogError(ex);
                }
            }
        }
    ...
    public void FixUpAtlas(UISprite sprite)
    {
        if (mReplacement == null)
        {
            if (name != null)
            {
                try
                {
                    ...
                    //if (mReplacement == null)
                        //AtlasManager.Instance.MarkSpriteForFixup(sprite, atlasId);
                }
                catch (Exception)
                {
                    //NotLogging this because we don't actually care
    ...
    public void FixUpAtlas(UIAtlas atlasForReplacement)
    {
        if (mReplacement == null)
        {
            mReplacement = atlasForReplacement;
        }
    }
}
