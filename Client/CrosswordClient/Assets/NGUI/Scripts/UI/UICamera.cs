...
public class UICamera : MonoBehaviour
{
	...
	static public UICamera eventHandler
	{
		get
		{
			...
			for (int i = 0; i < list.size; ++i)
			{
				...
				var compCamera = cam.gameObject.GetComponent<Camera>();
                if (maxDepthCam == null || (compCamera != null && maxDepthCam.mCam.depth < cam.mCam.depth))
                {
                    maxDepthCam = cam;
                }
            }
	...
	static public void Notify (GameObject go, string funcName, object obj)
	{
		...
		// JRA -- removed these messages since the camera was spamming them and we can't use hover or tooltip on mobile.
		if (funcName == "OnHover" || funcName == "OnTooltip")
		{
			return;
		}
#endif
		...
		{
			++mNotifying;
			//NGUISceneController.Instance.LogService.Log(LogLevel.Information,funcName + "(" + obj + ") on " + (go != null ? go.name : "<null>") + "; " + currentTouchID + ", " + Input.touchCount, this);
    ...
    public static bool BlockUI { get; set; }
